...
export default class ListCategories extends Component {
  ...
  render() {
    ...
    const { changeCategory, categorypil } = this.props
    return (
      <Col md={2} mt="2">
        <h4>
        ...
        <ListGroup>
          {categories && categories.map((category) => (
            <ListGroup.Item key={category.id} onClick={() => changeCategory (category.nama)} className={categorypil === category.nama && "category-aktif"}>
